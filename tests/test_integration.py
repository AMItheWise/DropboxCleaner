...
def test_integration_team_owned_only_excludes_member_homes(tmp_path: Path) -> None:
    team_discovery = make_team_discovery()
    backend = FakeDropboxBackend(
        [
            make_file(
    ...
    result = RunOrchestrator(adapter_factory=fake_adapter_factory(backend)).run(
    ...
    manifest_text = (Path(result.run_dir) / "manifest_dry_run.csv").read_text(encoding="utf-8")
    assert "ns:ns-root/root-plan.docx" in manifest_text
    assert "ns:ns-home-alice/Projects/old.psd" not in manifest_text
